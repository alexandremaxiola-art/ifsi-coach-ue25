...
async function togglePartValidation(topic,type){
  ...
  const d=await secureCall('student_validate',{ue:'25',topic,type});
  validationMap=d.moduleValidations||{};
  legacyValidated=d.validatedModules||[];
  validationLoadedFor=part.id;
  refreshValidationUI();
}
...
Promise.all(['./legacy-1.txt','./legacy-2.txt','./legacy-3.txt'].map(p=>nativeFetch(p+'?v=20260919-final',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('legacy '+r.status);return r.text()}))).then(parts=>{(0,eval)(parts.join(''));installUI()}).catch(()=>document.body.insertAdjacentHTML('beforeend','<div style="position:fixed;inset:20px;z-index:999;background:white;padding:30px;border-radius:18px">Impossible de charger IFSI Coach UE 2.5. Actualise la page.</div>'));
})();
